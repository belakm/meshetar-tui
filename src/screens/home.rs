  ...
  fn update(&mut self, action: Action) -> Result<Option<Action>> {
    match action {
      ...
      Action::Move(direction) => match direction {
        ...
        MoveDirection::Down => {
          self.selected_action = self.selected_action.saturating_add(1).min(HomeAction::COUNT - 1);
        },
        _ => {},
      },
      Action::Accept => {
        if let Some(command_tx) = &self.command_tx {
          let screen_id = HomeAction::iter().nth(self.selected_action).map(|s| s.to_screen_id());
  ...
  fn draw(&mut self, f: &mut Frame<'_>, area: Rect) -> Result<()> {
    ...
    for (index, action) in HomeAction::iter().enumerate() {
      ...
      let inner_layout = Layout::default()
        ...
        .constraints(vec![Constraint::Length(padding), Constraint::Length(button_width), Constraint::Min(padding)])
        .split(inner_area);
      let is_selected = index == self.selected_action;
      let button = Paragraph::new(action.to_string()).alignment(Alignment::Center).block(stylized_button(is_selected));
