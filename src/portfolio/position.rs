use crate::{
  assets::{MarketEvent, MarketEventDetail, Pair, Side},
...
impl PositionBuilder {
  ...
  pub fn position_id(self, value: PositionId) -> Self {
    Self { position_id: Some(value), ..self }
  }

  pub fn instrument(self, value: Pair) -> Self {
  ...
  pub fn build(self) -> Result<Position, PortfolioError> {
    Ok(Position {
      ...
      asset: self.asset.ok_or(PortfolioError::BuilderIncomplete("instrument"))?,
...
impl Default for PositionMeta {
  fn default() -> Self {
    Self { enter_time: Utc::now(), update_time: Utc::now(), exit_balance: None }
  }
}
